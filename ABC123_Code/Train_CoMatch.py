...
parser.add_argument('--data', metavar='DIR', default='/content/dataset', help='path to dataset')
parser.add_argument('--arch', metavar='ARCH', default='resnet50',choices=['resnet50','resnet50x2','resnet50x4'])
...
parser.add_argument('--batch-size-u', default=256, type=int,
...
parser.add_argument('--thr', default=0.5, type=float, help='pseudo-label confidence threshold')
parser.add_argument('--contrast-th', default=0.2, type=float, help='pseudo-label graph connection threshold')
parser.add_argument('--lam-u', default=10, type=float, help='weight for unsupervised cross-entropy loss')
parser.add_argument('--lam-c', default=3, type=float, help='weight for unsupervised contrastive loss')
parser.add_argument('--alpha', default=0.9, type=float, help='weight for model prediction in constructing pseudo-label')
parser.add_argument('--exp_dir', default='/content/drive/MyDrive/2572 FInalProject/CoMatch/comatch/checkpoints5%label', type=str, help='experiment directory')

## dataset settings
parser.add_argument('--percent', type=int, default=5, choices=[1,5,7.5,10], help='percentage of labeled samples')
...
def train(labeled_loader, unlabeled_loader, model, criteria_x, optimizer, epoch, args, logger, tb_logger):
    ...
    for i, unlabeled_batch in enumerate(unlabeled_loader):
        ...
        if i % args.print_freq == 0 and args.gpu==0:   
            ...
            print("{} || epoch:{}, iter: {}. loss_u: {:.3f}. loss_x: {:.3f}. loss_c: {:.3f}. "
                        "n_correct_u: {:.2f}/{:.2f}. n_edge: {:.3f}. LR: {:.2f}. "
                        "batch_time: {:.2f}. data_time: {:.2f}.".format(
                            args.exp_dir, epoch, i + 1, loss_u_meter.avg, loss_x_meter.avg, loss_contrast_meter.avg,
                            n_correct_u_lbs_meter.avg, n_conf.avg, pos_meter.avg, lr_log, batch_time.avg, data_time.avg))
    ...
    if args.gpu==0:    
        ...
        if n_conf.avg==0:
          guess=n_correct_u_lbs_meter.avg/(n_conf.avg+1e-20)
        else:
          guess=n_correct_u_lbs_meter.avg/n_conf.avg
        tb_logger.log_value('guess_label_acc', guess, epoch)
...
def validate(val_loader, model, args, logger, tb_logger, epoch):
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(val_loader):
            ...
            if i % args.print_freq == 0 and args.gpu==0:
                ...
                print("validation ||epoch:{}, iter: {}. acc1 : {:.2f}. acc5 : {:.2f}.".format(
                    epoch, i + 1, top1.avg, top5.avg))
    ...
    if args.gpu==0:    
        ...
        print("validation ||epoch:{}, acc1 : {:.2f}. acc5 : {:.2f}.".format(
